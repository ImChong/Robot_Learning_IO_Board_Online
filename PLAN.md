...
> **实施状态**：**M0–M5 已完成**，页面可运行（`python3 -m http.server 8080`），
> 收录 13 个项目 · 624 个节点 · 66 个奖励项（M8 补了 Isaac Lab 官方示例四张图）。相对本文的三处偏差：
...
> **给读 §4 / §5 的人**：数据模型已经落成 `schema/project.schema.json` + `data/taxonomy.json`，
> 九份真实数据在跑。所以 §4 / §5 里为 MimicKit 提出的每一处扩展**都是对已上线结构的增量**，
> 不是绿地设计；枚举值必须同时登记进 `data/taxonomy.json`（校验脚本从那里读词汇表，不是从本文读）。
> 文中用「M0 已用……实现，本文的提法作废」标出已被实现取代的提法，
> 用「⟨M5 已落地⟩」标出接 MimicKit 时真正做掉的扩展。
...
| 要改的文件 | 改什么 | 风险 |
|---|---|---|
| `schema/project.schema.json` | 项目顶层是 `additionalProperties: false`，所以显式加了 `inherits` / `overrides` / `diffSummary`，并把 `modes` 从必填改成可选（继承项目不写 `modes`） | 低，纯声明 |
| `src/data.js` | `loadProject(id)` 发现 `inherits` 就递归取父项目再合并，合并结果进缓存 | 父项目走同一份缓存与 `inflight` 表，所以「同时切两个共享同一父项目的子项目」不会重复拉父文件 |
| `scripts/validate.mjs` | 先把全部文件读进内存，再对继承项目做合并后校验；另加三条检查：继承不超过一层、父项目已在注册表登记、`overrides` 的路径与 id 确实命中父项目里存在的东西 | 最后一条最有用——补丁写错节点 id 会静默失效，不检查根本发现不了 |

「两份合并逻辑必须一致」是这里真正的隐患，所以合并写成了不依赖 DOM 与 `fetch` 的纯函数模块 **`src/inherit.js`**，页面与校验脚本 import 同一份。`scripts/validate.mjs` 本来就是 Node ESM，import 一个纯函数没有障碍。

补丁格式最终定成这样（`awr.json` 的实际写法）：`facts` 按 label 覆盖 value，`nodes` / `rewards` 按 id 浅合并，配套 `nodes+` / `nodes-` / `rewards+` / `rewards-` / `edges+` / `edges-` 做增删。合并不改动父项目对象（内部先深拷贝），所以父子项目可以同时打开。
...
| hover 节点 | 高亮该节点的完整上下游链路，其余节点降透明度。**⟨M7 后⟩** 链路上的流动光点同步加亮加快 |
| 连线数据流动 | **⟨M7 后⟩** 每条连线上有一串光点顺着数据流向走，相位按起点所在的列错开，整张图看起来是一波数据从输入侧推到输出侧。实现是叠在连线上的第二条同 `d` 路径，只推 `stroke-dashoffset`，一个动画周期正好推进一个虚线周期——所以边多长都无缝循环，全部边共用一条 keyframes，速度天然一致，不必按长度分别算时长。压暗、筛选、讲解的明暗靠同一套 `data-from` / `data-to` 跟着走：讲过的边才有光点在流。工具栏 `流动` 或 `L` 开关，选择存 `localStorage`，系统开了「减少动态」时默认关着 |
| click 节点 | 右侧抽屉展开详情；URL 同步 `?p=sonic&mode=train&n=obs.gravity_dir`，可分享可回退 |
| click 连线 | **⟨M7 后⟩** 点线或线上的标注，抽屉换成这条线的详情：哪一类流（说明取自 `taxonomy.json` 的 `edgeKinds[].desc`）、搬的是起点那个向量的多少维与什么分量顺序、前向还是绕通道的回路、两端有没有 `train-only`（有就是部署时消失）。图上只留它两端，其余压暗。边在数据里只有 `from` / `to` / `kind` / `style` / `label` 五个字段，上面这些话大半是从两端的节点与泳道次序推出来的——推得出来就不必往数据里再抄一份。URL 记 `?e=from>to`，和 `edges-` 补丁同一套键；模块详情里新增「连线」一节列出全部上下游边，既是导航也是键盘唯一的入口（画布 SVG 是 `aria-hidden` 的，不该往里塞几十个可聚焦元素） |
...
| 键盘 | `P` 打开项目下拉、`[` `]` 前后切项目，`T` / `D` 切模式，`F` 复位画布，`Esc` 关抽屉（模块与连线都算）；**⟨M7 后⟩** `L` 开关连线流动 |
...
| 连线方向 | **⟨M7 后⟩** 线上流动的光点，走向即数据流向；箭头本来就在，但缩到「适应窗口」的比例后小得认不出，光点在动，一眼就分得清 |
...
- 落 `smp.json`（附录 C.8），启用 `task` 泳道。主图取 **dodgeball** 预设——它是三个任务里唯一给出外部感知输入的，另外两个（location / steering）作为批注说明。
- dodgeball 预设让输入五类的 D 类在全站第一次有内容（`taxonomy.json` 里 D 类的说明原本写着「本站首批两个项目都不使用本类」）。
...
- ~~移动端布局（画布改为纵向堆叠 + 折叠泳道）~~ ✅ **走的不是这条路**。动手时发现「纵向堆叠 + 折叠泳道」是在改这张图本身，而图的横向分层（输入 → 网络 → 输出）正是它要表达的东西，折掉就没得看了。落地的是三件不动布局的事：画布接管触摸手势（单指平移、双指捏合），`适应窗口` 在小画布上保留 0.52 的可读下限、贴左起排让读者顺着阅读方向拖，详情改成从底部升起的浮层并把画布顶到顶栏底下。**结论：窄屏要改的是取景方式，不是图的结构。**
- 深链接分享卡片、导出 SVG/PNG。
- 拖拽微调节点位置并导出坐标，回写 JSON。

**M6.1 项目排序改为按发布时间** ✅

注册表里手工编号的 `order` 换成 `published`（首次公开发表日期，有预印本以预印本首版为准）。原来的编号 1、2、11…17 恰好就是时间序，但那是人排出来的：加一个 2023 年的方法就得回头重排一串序号，而且页面上看不出顺序的依据是什么。现在依据写在数据里，`YYYY-MM` 显示在下拉列表右侧、完整日期显示在项目头，读者能核对。

分组仍是第一顺位，组内才按时间排。纯按时间排会让「真机路线」和「MimicKit 方法族」交错，而选择器的分组标题是按列表顺序插在每组第一项前面的——交错会让同一个标题重复出现好几次。**组是导航，时间是组内的读法。**

**M6.2 按代码运行顺序逐个模块讲解** ✅

节点图回答「有哪些模块、谁连谁」，但不回答「跑一拍的时候它们按什么顺序发生」。讲解补的是后者：从数据源开始，一格一格高亮到奖励与参数更新，讲过的保持正常亮度、没讲到的压暗，于是整条链路是随着讲解一格一格亮起来的。

两个决定：

1. **顺序从图本身推，不写进数据文件**（`src/tour.js`）。每个项目手抄一份步骤表，等于同一件事有两份要同步维护的描述，加节点时漏改一处就会讲到一个图上不存在的模块。推导规则只有两条：默认按读图顺序（泳道从左到右、泳道内按数据顺序），但一个模块的全部前置模块都讲完了它才能出场（就绪集合取读图顺序最靠前者的 Kahn 拓扑排序）。回路边不参与排序——它们是「下一拍的事」，算进去整张图就成了环；它们在解说里单占一行，正好说明这条线为什么绕回去。九个项目十八张图推出来的序列全部覆盖完整、无依赖倒置，且每张图的阶段数正好等于泳道数（说明推出来的顺序确实是一列讲完再进下一列）。
2. **解说词取自节点已有的 `desc` / `note` / `source`**，不另写一份。那是逐项核对过的内容，另写一份早晚对不上。面板补的是图上看得见、字段里没有的部分：上一步是谁、算完交给谁、哪条线绕回去了，以及这一列在整轮里干什么（`taxonomy.json` 的 `lanes` 新增 `desc`）。

`scripts/validate.mjs` 把「序列覆盖每个模块且只讲一次」钉成校验项：顺序推错了不一定看得出来，讲漏一个模块必须报错。
...
**M6.4 顶栏的 GitHub 入口在移动端保留** ✅

M6.3 让位顺序的最后一档是「480px 以下整个收掉 GitHub 链接，页脚还有一份」。真在手机上看过之后这条不成立：页脚在 492 个节点的画布下面，读者要找源码得先滑完整张图，而顶栏是粘性的——被收掉的恰恰是唯一随手够得着的那个入口。

改法是换形态而不是换位置：链接里加上 GitHub 的标记图形，720px 以下收掉「GitHub」五个字母、只留图标，做成和主题按钮同尺寸的方钮。文字那半约 48px，收掉之后净占 36px，360px 宽的机器上项目选择器仍有 208px（够显示完整项目名加「1/9」），480px 那档规则随之删掉。**图标认得出来的是同一个标记，不依赖读得懂那五个字母**，所以这一档丢的是字不是功能。顺带把触屏下顶栏右侧两个方钮的高度统一提到 36px，和 `.icon-btn` 那批一致。
...
### M8 — Isaac Lab 官方示例：两个任务 × 两种工作流 ✅

前九个项目回答的都是「这条研究路线吃什么、吐什么」。这一批回答的是另一个问题：**同一个任务换一种写法，到底换掉了什么**。

Isaac Lab 给 Cartpole 与 Ant 各留了两份官方实现——manager-based（观测/奖励/终止/事件写成 `ObsTerm` / `RewTerm` / `DoneTerm` / `EventTerm` 四种配置项，运行时由 Manager 扫）与 direct（写成 `_get_observations` / `_get_rewards` / `_get_dones` 三个方法）。这是本站第一次出现「同任务对照组」：两边的网络与 PPO 超参逐字段相同，所以图上每一处差异都只可能来自环境定义本身。

**为什么值得单独收一组**

原本只打算收 manager-based Ant 一张。逐行核对之后发现这四张图恰好构成一个 2×2，而且四个格子里没有一格是重复的：

| | Cartpole | Ant |
|---|---|---|
| **manager-based** | 4 维 `(x, θ, ẋ, θ̇)` · 2 个 ObsTerm | 60 维 · 11 个 ObsTerm |
| **direct** | 4 维 `(θ, θ̇, x, ẋ)` · 一句 `torch.cat` | 36 维 · 一句 `torch.cat` |

两条对照线各自给出一个结论：

- **Cartpole 那条**：维度相同、顺序相反。manager-based 按「位置整块 + 速度整块」拼（块内按 articulation 关节序），direct 手写成「摆杆整块 + 滑块整块」。四个位置里只有首尾恰好错开成不同的量，两边都不报错。加上 Gym 经典 CartPole 的 `(x, ẋ, θ, θ̇)`，三份实现三种排列。这是 §8 第 8 条（`dimLayout` 必填）能举出的最短反例——四个数就够了，不必等到 29 个关节。
- **Ant 那条**：60 = 36 + 24。两边观测的前 28 维逐位对齐，manager-based 多出来的正是四个足端的六维力旋量。同一个任务，两份官方实现对「策略需不需要知道脚上受了多大力」给出了相反的答案；而这 24 维在真机上要靠四套六维力/力矩传感器换来。

**顺带核出来的九处 MDP 差异（Ant 那对）**

「两种工作流的同一个环境」这句话只对了一半：写法是两种，环境也是两个。除观测外还有 episode 16 s vs 15 s、`env_spacing` 5.0 vs 4.0、复位随机关节 ±0.2 rad vs 完全不随机、目标点为世界系常量 vs 逐环境加原点偏移、`progress` 除以策略步长 vs 物理步长（数值差 2 倍）、电耗多乘一个 `dof_vel_scale=0.2`（力度差 5 倍）、限位惩罚 −0.1 连续量且两侧对称 vs −1.0 计数且只罚上限一侧、摔倒仅失去 0.5 存活分 vs 整份奖励被 −2.0 覆盖。**拿两边曲线比「哪种工作流更快」，比的其实是这九处。**这些差异一条都不写在文档里，只能逐行读代码，正是这块板存在的理由。

**数据口径上的两个决定**

1. **`published` 取上游 CHANGELOG 里的版本日期**，不是任务原型的论文日期。`Isaac-Cartpole-v0` / `Isaac-Ant-v0` 是 `isaaclab_tasks` 0.1.0（2023-01-17）的初始发布内容，两个 `-Direct-v0` 是 0.7.0（2024-05-07）加的。按 §8 第 1 条，这两个日期在仓库里读得到，1983 年的 cart-pole 论文与 2015 年的 GAE 论文则不是这四份实现的发表日期。于是组内顺序自然排成「2023 的两张 manager-based → 2024 的两张 direct」。
2. **蚂蚁的关节顺序只写到「4 髋在前、4 踝在后」**。Isaac Lab 按运动树广度优先展开，这一层确定；同层内的左右先后由 `ant_instanceable.usd` 的 prim 顺序决定，而该 USD 走 Nucleus 资产服务器、不随仓库分发。按 §8 第 6、8 条，宁可粗，不许编。8 这个数字本身是 `verified` 的——`direct/ant/ant_env.py` 里 `action_space = 8`、`joint_gears` 也正好 8 项。

**推理态：沿用 MimicKit 那七个项目的口径**

这四张图从头到尾没打算离开仿真器，所以第二个模式不是「部署态」而是「推理态」，观测一律标 `sim-only`（仿真直读、真机无对应通道，但推理时依然在用），推理态里**一格都不消失**——真正被摘掉的只有学习侧：Critic、奖励、终止、复位随机化。

这套口径不是新发明的：MimicKit 那七个动画路线的方法早就是这么写的，模式 id 用 `test`、`label` 写「推理态」，DeepMimic 的推理态同样是 11 个观测一个不少。所以这一批**沿用 `test` 而不是 `deploy`**——对比视图按 mode id 配对（`src/render-compare.js`），用 `deploy` 会让这四个项目在第二个模式下没法和最像它们的那七个对照。判断依据很直接：是否以真机为目标，决定第二个模式叫什么、也决定它该和谁比。

新写进去的是另一件事：推理态画的是 `export_policy_as_onnx` **真正导出的那段计算**，而 `play.py` 里奖励与终止照常算、只是没人用——这一点写在 `e.sim` 的备注里，避免把「导出图」误说成「运行时没有奖励」。

- 交付：4 个项目 · 132 个节点 · 24 个奖励项，校验零警告；`data/isaaclab/` 下四份数据文件，注册表新增 `isaaclab` 分组（排在最前，它是这块板上门槛最低的入口）。
